...
@export var follow_node : Node3D # Node to follow (assign this to your player)

var trauma : float = 0.0 # Current shake strength
var trauma_power : int = 10 # Trauma exponent. Increase for more extreme shaking
...
func add_trauma(amount : float) -> void:
	trauma = min(trauma + amount, .5) # Add the amount of trauma (capped at 1.0)
